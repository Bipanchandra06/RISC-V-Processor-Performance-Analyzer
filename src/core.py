...
class SingleStageCore(Core):
    ...
    def step(self):
        ...
        self.trace.record(self.cycle, self.state.IF["PC"], {"IF": self.state.IF, "ID": self.state.ID, "EX": self.state.EX, "MEM": self.state.MEM, "WB": self.state.WB}, self.state.ID["Instr"], self.events)
        self.events = []
        self.cycle += 1
...
class FiveStageCore(Core):
    ...
    def id_stage(self):
        ...
        if stall:
            self.stall_count += 1
            self.events.append("load-use stall")
        ...
        if self.next_state.IF["PCSrc"]:
            self.branch_flushes += 1
            self.events.append("branch/jump flush")
            self.state.IF["Flush"] = True
            self.next_state.ID["nop"] = True

        if branch and self.predictor is not None:
            predicted = self.predictor.predict(self.state.ID["PC"])
            self.predictor.update(self.state.ID["PC"], bool(self.next_state.IF["PCSrc"]), predicted)
    ...
    def ex_stage(self):
        ...
        if forward_a or forward_b:
            self.forwarding_events += int(bool(forward_a)) + int(bool(forward_b))
            self.events.append(f"forward A={forward_a:02b}, B={forward_b:02b}")
        ...
        self.next_state.MEM["instr"] = self.state.EX["instr"]
    ...
    def mem_stage(self):
        ...
        self.next_state.WB["instr"] = self.state.MEM["instr"]
